function DotMatrixChart(dataset,options){
    ...
    var	margin = {top: dotRadius*10, right: dotRadius*10, bottom: dotRadius*15, left: dotRadius*10};

// rotate? < ^ >
    height = (dotRadius*2 + dotPaddingBottom + dotPaddingTop) * noOfCirclesInARow;
    width = numberOfLines * (dotRadius*2 + dotPaddingLeft + dotPaddingRight);

// rotate dot starting point <-> /\ \/
    // Set the ranges
    var	xScale = d3.scale.linear().range([margin.left, width]);
    var	yScale = d3.scale.linear().range([height, margin.bottom]);

//rotate x axis to left y axis
    var xAxis = d3.svg.axis()
          ...
          .tickSize(height-margin.top-(dotRadius*2), 0, 0);
...
//rotate dot display from x <--> y / add tick marks?
    xScale.domain([d3.max(dataset,function(d){return groupScale(d.group)+1;}),0]);
    yScale.domain([noOfCirclesInARow,0]);
    ...
    svg.append("g")
        .attr("class", "x axis")
//height adjustment of x axis
        .attr("transform", "translate(0," + (margin.top + dotRadius) + ")")
        .call(xAxis)
//Edit x axis labels
        .selectAll("text")
        .attr("y", -dotRadius*2)
        .attr("x", -(dotRadius + dotPaddingLeft + dotPaddingRight))
        .attr("dy", ".35em")
        .style("font-size", "15px")
    ...
    svg.append("text")
        ...
        .text("Development in US vs Asian Countries");
...
//dot layout x <--> y
            arr[i] = {x:groupScale(d.group)+globalLineNoForGroup[d.group],y: globalDotXPosition[d.group]-2, group:d.group,category:d.category};
    ...
    .append("g")
    .attr("class", "legend")
    .attr("transform", "translate(" + margin.right/3  + "," + ((margin.top*1.5)+dotRadius) + ")");
    ...
    legend
      ...
      .attr("y", function(d,i){return i*dotRadius*4 + dotRadius/2;})
      .style("font-size", "15px")
